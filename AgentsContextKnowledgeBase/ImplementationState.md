...
## Workstream (implemented in working tree 2026-07-18): Assisted Onboarding (Superadmin Gym Provisioning)

Spec: [ImplementationPlan-AssistedOnboarding.md](ImplementationPlan-AssistedOnboarding.md) · decision basis: [ADR 0005](../docs/adr/0005-platform-managed-gym-provisioning.md) · branch `super-admin` (from `polish-and-hardening` @ `b6e8f2fad1e20ccfd440b84b02cc6e4b91a1bc97`).

| Unit | Scope | Status |
|---|---|---|
| AO-P0 | Migration `027_assisted_onboarding.sql` (`gym_claim_invites`, `provisioning_runs`, `platform_onboarding_events`, `gyms.branch_name`, `provision_gym_workspace`/`claim_gym_ownership`/`supersede_claim_invite`/`mark_claim_invite_delivery`/`get_claim_invite_preview` RPCs, 4 new feature-key mirrors + additive enforcement in `join_gym`/`kiosk_checkin`/`kiosk_checkin_by_member`/`kiosk_get_occupancy`) + hand-extended `lib/database.types.ts` | Implemented (Fable 5, working tree, 2026-07-18) — 14 static SQL-contract tests green; migration not applied to any live database (no Docker/local Supabase in this environment — see limitations) |
| AO-P1 | Platform-admin access layer: `lib/platform-admin.ts`, middleware `/superadmin` branch (reuses `app_metadata.platform_role`, no new DB role) + `/claim` public allowlist, `app/superadmin/layout.tsx` (defense-in-depth `notFound()`) + `SuperadminShell` (dark sidebar, one nav item, operator identity, logout) | Implemented — 26 authorization tests green (unauthenticated/non-operator/operator matrix via a mocked `@supabase/ssr`, real `middleware()` execution) |
| AO-P2 | Wizard core: `WizardProvider` reducer + session-storage mirror, `WizardTimeline` (keyboard arrow nav, `aria-current`), `SetupTimeRow`, `PreviewColumn` (3 live previews + click-to-expand `Modal`), Step 1 Gym (slug auto-derive/edit, debounced `slug-check`, logo upload+fallback) | Implemented — 40+ unit tests green (state reducer, slug rules, Step 1 blocking/validation) |
| AO-P3 | Step 2 Owner & Staff: PH-mobile validation, consent method, duplicate-email guard (owner/staff), `email-check` route + existing-account/multi-gym/pending-invite banners, repeatable staff entries | Implemented — 8 tests green |
| AO-P4 | Step 3 Plan & Access: plans (≥1, cannot remove last), operating hours (default 5–10, copy-to-days, ≥1 open day), 8 access switches incl. 4 new feature keys, `MemberImportSection` (CSV template/parse/preview, all-or-explicit-confirm, row-numbered errors, in-file duplicate detection) | Implemented — 20 tests green (8 plan/access + 4 CSV lib + 4 import-section + 4 schema) |
| AO-P5 | Step 4 Review & Invite (clickable rows, green/amber states) + atomic/idempotent `provision` route (account resolution, RPC call, non-critical logo upload, non-critical email dispatch) + `SuccessState` (pending-claim status, copy link, resend, truthful delivery-failure state) | Implemented — 9 provision-route tests + 5 review + 5 success-state tests green, incl. idempotency-key reuse across a retried submission and the 207 recoverable-email-failure path |
| AO-P6 | Public `/claim/[token]` page + `ClaimConfirmClient` (signed-out CTA, wrong-email warning with masked email, explicit "Claim ownership of…" confirm) + `/api/claim/accept` (distinct error codes for expired/used/superseded/wrong-email) + `resend-invite` route (supersedes old token) | Implemented — 8 claim-accept + 5 resend-invite + 4 claim-confirm-client tests green |
| AO-P7 | Accessibility (live regions, `aria-current`/`aria-label` throughout, reduced-motion CSS, color-plus-icon/text status) + responsive (preview column hidden below `lg`) + full quality gate | Implemented — `npm run lint` (repo-wide) ✅, `npx tsc --noEmit` ✅ (zero new errors; pre-existing `.next/**` stale-artifact noise unchanged), complete unit/integration suite **574/574** ✅, production build ✅ (all new routes generated: `/superadmin`, `/superadmin/onboarding/new`, `/claim/[token]`, 4 new `/api/superadmin/onboarding/*` routes, `/api/claim/accept`) |

**117 feature-specific tests** (15 files) plus the complete pre-existing suite all green together. **Known limitations** (see plan §30 risk section, updated): (1) migration 027 was never applied to any database — no Docker/local Supabase available in this environment, so it is verified only by static SQL-contract assertions and careful line-by-line review against the actual deployed schema (019/020/023/025/026), not by a live `db:reset`/`db:invariants` run; (2) Playwright E2E was not executed (no `E2E_PLATFORM_ADMIN_*` fixture exists and the full suite was out of scope for this pass); (3) after a signed-out visitor completes sign-in from `/claim/[token]`, the existing global post-auth router does not return them to the claim page — they must reopen the claim link (documented deviation, chosen over rewiring the shared auth router). No commits, pushes, or hosted changes were made — all changes are staged in the working tree on `super-admin` for the developer to review and commit.
