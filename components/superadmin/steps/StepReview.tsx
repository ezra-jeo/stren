...
function buildReviewRows(draft: ReturnType<typeof useWizard>['state']['draft']): ReviewRow[] {
  ...
  return [
    ...
    },
    {
      ...
      lines: [draft.owner.name || 'Owner not named', 'Owner', `${draft.staff.length} staff member${draft.staff.length === 1 ? '' : 's'}`],
    },
    {
    ...
    { step: 'planAccess', title: 'Operating hours', status: 'complete', lines: [hoursSummary] },
    {
      ...
      lines: [
        ...
        `Staff manual check-in: ${draft.switches.staffManualCheckin ? 'On' : 'Off'}`,
        `Occupancy count: ${draft.switches.occupancyCount ? 'On' : 'Off'}`,
        'Visibility: Private until the owner publishes in Studio',
      ],
    },
    {
