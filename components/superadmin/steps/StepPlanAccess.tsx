...
const SWITCH_ROWS: Array<{
  key: 'kioskCheckin' | 'generateInviteQr' | 'autoApproveJoins' | 'staffManualCheckin' | 'checkinRequiresMembership' | 'occupancyCount';
  label: string;
  help: string;
}> = [
  ...
  { key: 'autoApproveJoins', label: 'Auto-approve members joining via gym QR', help: 'Off by default — new joins stay pending until staff review them.' },
  { key: 'staffManualCheckin', label: 'Allow staff manual check-in', help: 'Staff can check a member in without a QR scan.' },
  { key: 'checkinRequiresMembership', label: 'Require active membership for check-in', help: 'Kiosk blocks check-in when a membership has lapsed.' },
    ...
    <div className="onboarding-step-enter space-y-6">
      ...
      <section className="space-y-3 pt-3" style={{ borderTop: `1px solid ${A.border}` }}>
        ...
        <div className="py-1.5">
          <span className="block text-xs font-medium mb-1.5" style={{ color: A.text }}>Gym visibility</span>
          <ChoicePicker
            value={switches.visibility}
            onChange={(value) => dispatch({ type: 'setSwitches', patch: { visibility: value } })}
            options={[
              { value: 'private', label: 'Private', sub: 'Unlisted — only reachable via direct link or QR' },
              { value: 'public', label: 'Public', sub: 'Visible on Stren discovery surfaces' },
            ]}
          />
        </div>
      </section>
