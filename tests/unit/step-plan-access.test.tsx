...
describe('StepPlanAccess — access switches', () => {
  it('renders the eight switch rows with documented defaults', () => {
    renderStep();
    ...
    expect(screen.getByLabelText('Auto-approve members joining via gym QR')).not.toBeChecked();
    expect(screen.getByLabelText('Allow staff manual check-in')).toBeChecked();
    expect(screen.getByLabelText('Require active membership for check-in')).toBeChecked();
