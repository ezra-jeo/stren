...
beforeEach(() => {
  vi.stubGlobal('fetch', vi.fn(async () => ({
    ok: true,
    status: 200,
    json: async () => ({ gymId: 'gym-1', gymName: 'Iron Fitness', gymCode: 'iron-fitness', ownerEmail: 'jane@example.com', expiresAt: '2026-07-18T00:00:00.000Z', claimLink: 'https://stren.app/claim/tok', emailDelivered: true }),
  })));
});
...
describe('StepReview', () => {
  ...
  it('Finish setup calls the provision API and reports success via onFinished', async () => {
    ...
    await waitFor(() => expect(onFinished).toHaveBeenCalledWith(expect.objectContaining({ gymId: 'gym-1', claimLink: 'https://stren.app/claim/tok' })));
  });
