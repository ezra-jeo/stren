...
-- Remediates findings from the 2026-06-17 audit.
-- All changes are idempotent (DROP ... IF EXISTS / CREATE OR REPLACE).
-- ============================================================

-- ────────────────────────────────────────────────────────────
-- H2  gyms_public_select USING (true) → own gym only
--     Public discovery is served by the SECURITY DEFINER
--     get_gym_by_code / search_gyms helpers which bypass RLS.
-- ────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────
-- C1  profiles_update missing WITH CHECK → privilege escalation
--     Also add a BEFORE UPDATE trigger that hard-blocks
--     non-managers from mutating role / status / gym_id.
-- ────────────────────────────────────────────────────────────
...
AS $$
BEGIN
  -- Members can only modify their own display fields — never role/status/gym_id.
...
-- ────────────────────────────────────────────────────────────
-- H1  handle_new_user trusts raw_user_meta_data->>'role'
--     Hardcode 'member'; owner creation goes via create_gym_and_owner RPC.
-- ────────────────────────────────────────────────────────────
...
BEGIN
  ...
  VALUES (
    ...
    'member',  -- role is always member; owners are promoted via create_gym_and_owner RPC
    'pending',
...
-- ────────────────────────────────────────────────────────────
-- H3  notifications_insert / notifications_update not manager-gated
-- ────────────────────────────────────────────────────────────
...
-- Managers can update any notification; members can only update their own
-- (e.g., mark as read) — kept via the existing notifications_update_own policy.
CREATE POLICY notifications_update ON public.notifications FOR UPDATE
  USING (gym_id = public.get_gym_id() AND public.is_manager());

-- ────────────────────────────────────────────────────────────
-- M4  attendance_insert / attendance_update missing gym scope
--     streaks_manage missing gym scope
-- ────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────
-- C3  dev_all_classes / dev_all_enrollments / dev_all_checkins USING (true)
--     These tables are unused by app code (only in repo dumps).
--     Lock them to manager-only + gym scope.
-- ────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────
-- C4  Kiosk RPCs — add auth + is_manager() + gym scope
--     Also: REVOKE EXECUTE FROM PUBLIC/anon, GRANT TO authenticated
--     kiosk_search_members: escape ILIKE wildcards
-- ────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Look up member by QR code, scoped to the caller's gym
...
BEGIN
  ...
  -- Member must belong to the caller's gym
  SELECT * INTO v_member
  FROM public.profiles
...
BEGIN
  ...
  -- Only check out attendance records belonging to this gym
